...
class ShoutmeloadSpider(scrapy.Spider):
    name = "brooklynbicycleco"
    from_date = datetime.date.today() - datetime.timedelta(6*365/12)
    allowed_domains = ["brooklynbicycleco.com"]
    start_urls = [
        "http://blog.brooklynbicycleco.com"
    ]
    ...
    def parse_dir_contents(self, response):
        ...
        item['contents'] = (" ").join(response.xpath('//div[contains(@class, "blog_content")]/p/text()').extract()).strip()
        yield item