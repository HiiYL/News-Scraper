...
class ShoutmeloadSpider(scrapy.Spider):
    ...
    allowed_domains = ["chillopedia.com"]
    start_urls = [
        "http://chillopedia.com/"
    ]
    ...
    def parse(self, response):
        ...
        next_page = response.xpath('//span[@class="current"]/following-sibling::a[1]/@href')
        if next_page:
    ...
    def parse_dir_contents(self, response):
        ...
        item['contents'] = ("").join(response.xpath('//div[contains(@class, "texty")]/p//text()').extract()).strip()
        yield item